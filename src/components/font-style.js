...
const FontStyle = ({ value, onChange }) => {
  ...
  const calculateMargin = (numIcons) => {
    return numIcons * 14 + "px"; 
  };

  const iconMargin =
    selectedIcons.length > 0 ? calculateMargin(selectedIcons.length) : "0";
            ...
            {selectedIcons.length > 0 ? (
              selectedIcons.map((icon, index) => (
                <Icon
                  key={index}
                  icon={icon}
                  style={{ right: "-57px", fill: "var(--primary-color)" }}
                />
              ))
            ) : (
              <></>
            ...
            <div className="popow-content-ico-position">
              {styleOptions.map((option) => (
                <Button
                  ...
                  label={option.label}
                  className={
                    value[option.type] === option.value ? "selected" : ""
                  }
                >
