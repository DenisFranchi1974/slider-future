...
const MenuControls = ({
      ...
      slide.id === slideId
        ? {
            ...
            elements: slide.elements.filter(
              (element, i) => !(element.type === "menu" && i === index)
            ),
          }
        : slide
    );
  ...
  const [menuItems, setMenuItems] = useState(
    element.menuItems || [{ text: "Home", link: "",openInNewTab: false, subMenu: [] }]
  );
  
  const addMenuItem = () => {
    const newItem = { text: `Item ${menuItems.length + 1}`, link: "",openInNewTab: false, subMenu: [] }; // Rimuovi opensInNewTab
  ...
  const addSubMenuItem = (index) => {
    const newSubItem = { text: `Sub Item ${menuItems[index].subMenu.length + 1}`, link: "", openInNewTab: false };
    const updatedMenuItems = [...menuItems];
    updatedMenuItems[index].subMenu.push(newSubItem);
    setMenuItems(updatedMenuItems);
    ...
    // Aggiorna anche nel contesto delle slide se necessario
};

  
const updateMenuItem = (index, key, newValue, subIndex) => {
  const updatedMenuItems = [...menuItems];
  
  if (subIndex !== undefined) {
    if (key === "subMenu") {
      updatedMenuItems[index].subMenu[subIndex].text = newValue;
    } else if (key === "link") {
      updatedMenuItems[index].subMenu[subIndex].link = newValue;
    } else if (key === "openInNewTab") {
      updatedMenuItems[index].subMenu[subIndex].openInNewTab = newValue;
    }
  } else {
    updatedMenuItems[index][key] = newValue;
  }
  ...
  const removeSubMenuItem = (menuIndex, subIndex) => {
    const updatedMenuItems = [...menuItems];
    updatedMenuItems[menuIndex].subMenu = updatedMenuItems[menuIndex].subMenu.filter((_, idx) => idx !== subIndex);
    setMenuItems(updatedMenuItems);
 ...
 // Update z-index
 const updateZIndexMenu = (slideId, index, value) => {
  const updatedSlides = slides.map((slide) =>
    slide.id === slideId
      ? {
          ...slide,
  ...
  const updateDirection = (slideId, index, value) => {
    ...
    // Update menu mode
    const updateMenuMode = (slideId, index, value) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
    ...
    // Funzione per aggiornare la modalità di apertura del submenu
const updateSubmenuMode = (slideId, index, value) => {
  const updatedSlides = slides.map((slide) =>
    slide.id === slideId
      ? {
          ...slide,
  ...
  setAttributes({ slides: updatedSlides });
    ...
    // Update menu mode range responsive
    const updateBreakpoint = (slideId, index, value) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
  ...
  // Orientation
  const updateOrientationMenu = (slideId, index, value) => {
    const updatedSlides = slides.map((slide) =>
      slide.id === slideId
        ? {
            ...slide,
    ...
    <div className="custom-block-added">
      ...
      <div className="title-block-added">
        ...
        <Button
            ...
            label={__("Remove Menu", "cocoblocks")}
            icon={trash}
          ></Button>
      ...
      {activeSection === "content" && (
        ...
        {menuItems.map((item, idx) => (
           <div key={idx} className="content-section-panel menu-control" style={{ padding: "0" }}>
            ...
            >
                <Tooltip  placement="top" text={__('Add sub item','slider')}>
             <button onClick={() => addSubMenuItem(idx)} label={__("Add sub item", "slide")}>
             ...
             {item.subMenu && item.subMenu.length > 0 && item.subMenu.map((subItem, subIdx) => (
            <div key={subIdx} className="sub-menu-control">
             ...
             <div className="custom-select select-text-control">
              <TextControl
               label={
                ...
                <div className="content-text-input-menu">
                  <div className="title-element">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M200-280h560v-80H200v80Zm0-160h560v-80H200v80Zm0-160h400v-80H200v80Zm-40 440q-33 0-56.5-23.5T80-240v-480q0-33 23.5-56.5T160-800h640q33 0 56.5 23.5T880-720v480q0 33-23.5 56.5T800-160H160Zm0-80h640v-480H160v480Zm0 0v-480 480Z"/></svg>
                  {__("Sub Item text", "cocoblocks")}
                  </div>
                  <div className="title-element">
                 
                          <Button
                        isDestructive
                        onClick={() => removeSubMenuItem(idx, subIdx)}
                        className="button-remove-element"
                        label={__("Remove Sub Item", "cocoblocks")}
                        icon={trash}
                      ></Button>
              ...
              value={subItem.text}
              onChange={(newValue) => updateMenuItem(idx, "subMenu", newValue, subIdx)} // Funzione da definire
                placeholder={__("Add text...", "cocoblocks")}
              />
            </div>
            <div className="custom-select select-text-control">
              <LinkControl
                value={{ url: subItem.link }}
                onChange={(newLink) => {
                  updateMenuItem(idx, "link", newLink.url, subIdx);
                }}
              />
            </div>
            <div className="custom-select">
              <ToggleControl
                label={__("Open link in new tab?", "cocoblocks")}
                checked={subItem.openInNewTab}
                onChange={(newValue) => updateMenuItem(idx, "openInNewTab", newValue, subIdx)}
              />
            </div>
            ...
            <SelectControl
                label={
                  <>
                    ...
                    {__("Overlay Menu", "cocoblocks")}
                  </>
                }
          value={element.menuMode}
          options={[
            { label: __('Off','cocoblock'), value: 'normal' },
            { label: __('Always','cocoblock'), value: 'toggle' },
            { label: __('Mobile','cocoblock'), value: 'responsive' },
          ]}
          onChange={(value) =>
            updateMenuMode(slide.id, elementIndex, value)
          }
        />
        <Tooltip
                  placement="top"
...
{element.menuMode === 'responsive' && (
  <div className="custom-select">
          <RangeControl
            label={
              <>
               <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M120-120v-520h200v-200h520v720H120Zm520-80h120v-560H400v120h240v440Zm-240 0h160v-360H400v360Zm-200 0h120v-360H200v360Zm440-440v80-80Zm-320 80Zm240 0Zm80-80Z"/></svg>
                {__("Breakpoint (px)", "cocoblocks")}
              </>
            }
            value={element.breakpoint}
            onChange={(value) =>
              updateBreakpoint(slide.id, elementIndex, value)
            }
            min={320}
            max={1024}
          />
           <Tooltip
                  ...
                  text={__(
                    "Adjust the Range Control to set the pixel width for switching from the standard menu to the toggle menu.",
                    "cocoblocks"
                  )}
                >
                  <Icon icon={info} className="tooltip-icon" style={{top:'2px'}}/>
                </Tooltip>
          </div>
        )}
            <div className="custom-select select-control-label-right">
          ...
          (element.direction === "right" && (element.directionMenu === "menu-top" || element.directionMenu === "menu-bottom"))
            ...
            <div className="custom-select select-control-label-right">
              ...
              <SelectControl
                label={
                  <>
                   <svg style={{transform: 'rotate(90deg)'}} xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm280-80h280v-560H480v560Z"/></svg>
            ...
            <div className="custom-select select-control-label-right">
              <SelectControl
                label={
                  <>
                   <svg style={{transform: 'rotate(90deg)'}} xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M200-120q-33 0-56.5-23.5T120-200v-560q0-33 23.5-56.5T200-840h560q33 0 56.5 23.5T840-760v560q0 33-23.5 56.5T760-120H200Zm280-80h280v-560H480v560Z"/></svg>
                    {__("Menu Orientation", "cocoblocks")}
                  </>
                }
                value={element.orientationMenu}
                onChange={(value) =>
                  updateOrientationMenu(slide.id, elementIndex, value)
                }
                options={[
                  {
                    label: __("Horizontal", "cocoblocks"),
                    value: "row",
                  },
                  {
              ...
              <div className="custom-select select-control-label-right">
              <SelectControl
                label={__("Submenu Mode", "cocoblocks")}
                value={element.submenuMode || 'click'}
                options={[
                  { label: __('Click', 'cocoblock'), value: 'click' },
                  { label: __('Hover', 'cocoblock'), value: 'hover' },
                ]}
                onChange={(value) => updateSubmenuMode(slide.id, elementIndex, value)}
              />
           </div>
          ...
          <div className="content-section-panel" style={{ padding: "0" }}>
            <div className="custom-select">
              <RangeControl
                ...
                value={element.zIndexMenu}
                onChange={(value) =>
                  updateZIndexMenu(slide.id, elementIndex, value)
                }
                min={0}
                max={999}
                step={1}
