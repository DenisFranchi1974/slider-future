...
const DivControlsHover = ({
  ...
  const updateSlideDivColorHover = (slideId, index, color) => {
  ...
  // Update Rotate
  const updateRotateHoverDiv = (slideId, index, rotate) => {
    const updatedSlides = slides.map((slide) =>
      slide.id === slideId
        ? {
            ...slide,
    ...
    // Update border style
    const updateBorderStyleHoverDiv = (slideId, index, newBorderStyle) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
              elements: slide.elements.map((element, i) =>
                element.type === "div" && i === index
                  ? { ...element, borderStyleHoverDiv: newBorderStyle }
                  : element
              ),
            }
          : slide
    ...
    const updatedSlides = slides.map((slide) =>
      slide.id === slideId
        ? {
            ...
            elements: slide.elements.map((element, i) =>
              element.type === "div" && i === index
                ? { ...element, opacityHoverDiv: opacity }
                : element
            ),
          }
        : slide
  ...
  const updateDurationEffectHoverDiv = (slideId, index, newDurationEffect) => {
    const updatedSlides = slides.map((slide) =>
      slide.id === slideId
        ? {
            ...
            elements: slide.elements.map((element, i) =>
              element.type === "div" && i === index
                ? { ...element, durationEffectHoverDiv: newDurationEffect }
                : element
            ),
          }
        : slide
    ...
    // Update Border color
    const updateDivBackgroundBorderColorHover = (slideId, index, color) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
              elements: slide.elements.map((element, i) =>
                element.type === "div" && i === index
                  ? { ...element, backgroundBorderColorHoverDiv: color }
                  : element
              ),
            }
          : slide
    ...
    // Update Annimation
    const  updateDivAnimationHover = (slideId, index, animation) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
              elements: slide.elements.map((element, i) =>
                element.type === "div" && i === index
                  ? { ...element, animationHoverDiv: animation }
                  : element
              ),
            }
          : slide
    ...
    // Update Effect color
    const updateSlideEffectColorHoverDiv = (slideId, index, color) => {
      const updatedSlides = slides.map((slide) =>
        slide.id === slideId
          ? {
              ...slide,
              elements: slide.elements.map((element, i) =>
                element.type === "div" && i === index
                  ? { ...element, effectHoverColorHoverDiv: color }
                  : element
              ),
            }
          : slide
...
const updateTranslateEffectHoverDiv = (slideId, index, newEffect) => {
  const updatedSlides = slides.map((slide) =>
    slide.id === slideId
      ? {
          ...
          elements: slide.elements.map((element, i) =>
            element.type === "div" && i === index
              ? { ...element, translateEffectHoverDiv: newEffect }
              : element
          ),
        }
      : slide
  ...
  setAttributes({ slides: updatedSlides });
    ...
    const isAnimationNone = element.animationHoverDiv === " ";
    const showColorOptionsPanel = [
      "hover-effect-3-div-",
      "hover-effect-3-div",
      "hover-effect-4-div",
    ].includes(element.animationHoverDiv);
    const showTranslateRangeControl = [
      "hover-effect-2-div", // Translate Y
      "hover-effect-5-div", // Translate X
    ].includes(element.animationHoverDiv);
  ...
  return (
    <div className="custom-block-added">
          ...
          <div className="custom-select color">
              <ColorOptionsPanel
                colorNormal={element.divColorHover}
                setColorNormal={(color) =>
                  updateSlideDivColorHover(slide.id, elementIndex, color)
                }
            ...
            <div className="custom-select select-control-label-right">
              <SelectControl
                ...
                value={element.borderStyleHoverDiv}
                options={[
                  {
                ...
                onChange={(newBorderStyle) =>
                  updateBorderStyleHoverDiv(slide.id, elementIndex, newBorderStyle)
                }
              />
            </div>
            {element.borderStyleHoverDiv !== "none" && (
            <>
            <div className="custom-select color">
              <ColorOptionsPanel
                colorNormal={element.backgroundBorderColorHoverDiv}
                setColorNormal={(color) =>
                  updateDivBackgroundBorderColorHover(
                    slide.id,
                    elementIndex,
                    color
                  )
            ...
            <div className="custom-select">
              <RangeControl
                ...
                value={element.opacityHoverDiv}
                onChange={(opacity) =>
                  updateOpacityHoverDiv(slide.id, elementIndex, opacity)
                }
                min={0.1}
                max={1}
                step={0.1}
            ...
            <div className="custom-select">
              <RangeControl
                ...
                value={element.rotateHoverDiv}
                onChange={(rotate) =>
                  updateRotateHoverDiv(slide.id, elementIndex, rotate)
                }
                min={0}
                max={360}
                step={1}
          ...
          <div className="content-title-custom-panel intermedy">
            ...
            <div className="content-section-panel" style={{ padding: "0",marginTop:"18px"}}>
            <div className="custom-select select-control-label-right">
              <SelectControl
                label={
                  <>
                ...
                value={element.animationHoverDiv}
                options={[
                  { label: "None", value: "none" },
                  { label: "Scale", value: "hover-effect-1-div" },
                  { label: "Translate Y", value: "hover-effect-2-div" },
                  { label: "Translate X", value: "hover-effect-5-div" },
                  { label: "Shadow", value: "hover-effect-3-div-" },
                  { label: "Shadow Two", value: "hover-effect-3-div" },
                  { label: "Shadow Three", value: "hover-effect-4-div" },
                ]}
                onChange={(animation) =>
                  updateDivAnimationHover(slide.id, elementIndex, animation)
                }
              />
            </div>
            {!isAnimationNone && (
            ...
            <div className="custom-select color">
              <ColorOptionsPanel
                colorNormal={element.effectHoverColorHoverDiv}
                setColorNormal={(color) =>
                  updateSlideEffectColorHoverDiv(slide.id, elementIndex, color)
                }
            ...
            <div className="custom-select">
              <RangeControl
                ...
                value={element.translateEffectHoverDiv}
                onChange={(newEffect) =>
                  updateTranslateEffectHoverDiv(
                    slide.id,
                    elementIndex,
                    newEffect
                  )
...
export default DivControlsHover;
